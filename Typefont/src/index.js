...
export const Typefont = (function(undefined) {
  /**
  ...
  const _symbolsToDomain = (first, second) => {
    for (let key in first) if (!second[key]) delete first[key];

    for (let key in second) if (!first[key]) delete second[key];
  };
      ...
      image
        ...
        .then(() => {
          if (textRecognitionBinarization) {
            ...
            if (_needReverse(image.data)) image.reverse();
          }

          const timeout = setTimeout(
            () => reject(`Unable to recognize ${url}`),
            textRecognitionTimeout * 1000
          );

          TesseractOCR(image.toDataURL())
            .then(res => {
  ...
  const _prepare = (url, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      console.log('before the req', fontsIndex);
      ...
      Promise.all([
        ...
        .then(res => {
          console.log('After the req', res[1]);
          resolve({
            recognition: res[0],
            fonts: res[1],
          });
      ...
      _prepare(url, options)
        .then(res => {
          ...
          const finalize = (name, val, font) => {
            ...
            result.push(meta);
            if (progress) progress(name, val, done / todo);
          ...
          for (const name of fonts) {
            FontStorage.requestFont(`${fontsDirectory}${name}/${fontsData}`)
              .then(font => {
                _symbolsToDomain(symbols, font.alpha);
                _compare(symbols, font.alpha, options)
                  .then(fin => {
                    console.log('just before finalize');
                    finalize(name, fin, font);
                  })
                  .catch(reject);
              })
              .catch(reject);
